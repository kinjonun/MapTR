...
def create_av2_infos_mp(root_path,
    ...
    log_ids = list(loader.get_log_ids())  # 训练集700 个
    for l in FAIL_LOGS:
        if l in log_ids:
            log_ids.remove(l)
    ...
    prev_level = sdb_logger.level                              # 记录当前日志级别
    sdb_logger.setLevel(logging.CRITICAL)                      # 将日志级别设置为CRITICAL

    # FIXME: need to check the order
    pool = Pool(num_multithread)               # 进程池
    fn = partial(get_data_from_logid, loader=loader, data_root=root_path, pc_range=pc_range)
    rt = pool.map_async(fn, log_ids)
    pool.close()                               # 关闭进程池
    pool.join()
    results = rt.get()                         # 现在 rt 是一个异步结果对象， 可以使用get()方法获取结果

    # results = []
    # for log_id in log_ids:
    #     result = get_data_from_logid(log_id, loader=loader, data_root=root_path, pc_range=pc_range)
    ...
    for _samples, _discarded in results:
        for i in range(len(_samples)):
            _samples[i]['sample_idx'] = sample_idx             # 加载数据集应该是按这个，每个lidar timestamp
            sample_idx += 1
        samples += _samples
...
def get_data_from_logid(log_id,
    ...
    discarded = 0
    # pdb.set_trace()
    log_map_dirpath = Path(osp.join(data_root, log_id, "map"))   # 'data/av2/sensor/test/0c6e62d7-...-03b13aa21f68/map'
    vector_data_fnames = sorted(log_map_dirpath.glob("log_map_archive_*.json"))      # glob() 匹配指定模式的文件或目录
    ...
    cam_timestamps = loader._sdb.per_log_lidar_timestamps_index[log_id]  # all lidar timestamps 156

    for ts in cam_timestamps:
        cam_ring_fpath = [loader.get_closest_img_fpath(
            log_id, cam_name, ts
        ) for cam_name in CAM_NAMES]  # 最靠近该lidar_timestamps的图片地址
        ...
        for i, cam_name in enumerate(CAM_NAMES):
            ...
            # PinholeCamera(ego_SE3_cam=SE3(rotation=array([[-2.52965741e-03, -3.35816781e-04,  9.99996744e-01],
            #                                               [-9.99996794e-01, -1.14372652e-04, -2.52969595e-03],
            #                                               [ 1.15221794e-04, -9.99999937e-01, -3.35526381e-04]]),
            #                               translation=array([ 1.6624252 , -0.00419634,  1.43641676]) ),
            #      intrinsics=Intrinsics(fx_px=1762.9395020167026, fy_px=1762.9395020167026, cx_px=771.9795837963757,
            #                        cy_px=1018.180418446096, width_px=1550, height_px=2048),
            #      cam_name='ring_front_center')
            cam_timestamp_ns = int(cam_ring_fpath[i].stem)  # 获取cam路径中的时间戳
            cam_city_SE3_ego = loader.get_city_SE3_ego(log_id, cam_timestamp_ns)  # CAM到全局
            cams[cam_name] = dict(
                img_fpath=str(cam_ring_fpath[i]),
                intrinsics=pinhole_cam.intrinsics.K,
                extrinsics=pinhole_cam.extrinsics,
                e2g_translation=cam_city_SE3_ego.translation,  # [ 714.22394961, 3033.65030628,  -23.41281618]
                e2g_rotation=cam_city_SE3_ego.rotation,
            )

        city_SE3_ego = loader.get_city_SE3_ego(log_id, int(ts))  # 自车到全局
        e2g_translation = city_SE3_ego.translation  # [ 714.225941  , 3033.58774084,  -23.41073377]
